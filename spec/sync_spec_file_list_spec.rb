...
describe 'SyncSpecFiles' do
  ...
  describe '#rewrite_travis_content' do
      ...
      let(:current_file_list) do
        ...
        'spec/test/new1.rb', 'spec/test2/new2.rb'] end
    let(:travis_yaml) { YAML::load(File.open('spec/documents/.travis.yml')) }
      
    before(:each) do
      SyncSpecFiles.rewrite_travis_content(travis_yaml, current_file_list) do
        SyncSpecFiles.send(:write_to_file, travis_yaml)
      end
    end
    ...
    it 'adds new files to the last line' do
      ...
      expect(last_line).to include('spec/test/new1.rb')
      expect(last_line).to include('spec/test2/new2.rb')
      expect(last_line).to include('spec/test/new3.rb')
    end
    ...
    it 'removes unused buckets' do
      expect(travis_yaml['env']['matrix'].length).to eq(4)
    end
  end
end